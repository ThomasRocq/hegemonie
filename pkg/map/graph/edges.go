...
func (ev SetOfEdges) edgeLess(i int, d Edge) bool {
	s := ev[i]
...
func (ev SetOfEdges) Get(src, dst uint64) *Edge {
...
func (ev SetOfEdges) Slice(markerSrc, markerDst uint64, max uint32) []Edge {
	tab := make([]Edge, 0)
	...
	needle := Edge{S: markerSrc, D: markerDst}
	for ; i < iMax; i++ {
